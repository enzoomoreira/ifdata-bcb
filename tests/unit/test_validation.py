"""Testes unitarios para os normalizadores de input em domain/validation.py."""
...
class TestNormalizeDates:
    def test_single_int(self) -> None:
        assert normalize_dates(202403) == [202403]

    def test_single_str_dash(self) -> None:
        assert normalize_dates("2024-03") == [202403]

    def test_single_str_compact(self) -> None:
        assert normalize_dates("202403") == [202403]

    def test_list_of_ints(self) -> None:
        assert normalize_dates([202401, 202402, 202403]) == [202401, 202402, 202403]

    def test_list_mixed_types(self) -> None:
        assert normalize_dates([202401, "2024-02", 202403]) == [202401, 202402, 202403]
...
class TestValidateCnpj8:
    ...
    def test_base8_formatada_e_aceita(self, entrada: str) -> None:
        ...
        assert validate_cnpj8(entrada) == esperado
...
class TestNormalizeInstitutions:
    def test_single_string_wraps(self) -> None:
        assert normalize_institutions("60872504") == ["60872504"]

    def test_valid_list(self) -> None:
        assert normalize_institutions(["60872504", "90400888"]) == [
            "60872504",
            "90400888",
        ]
    ...
    def test_invalid_in_list_raises(self) -> None:
        with pytest.raises(InvalidIdentifierError):
            normalize_institutions(["60872504", "abc"])

    def test_empty_list_accepted(self) -> None:
        assert normalize_institutions([]) == []


class TestNormalizeAccounts:
    def test_single_string_wraps(self) -> None:
        assert normalize_accounts("ATIVO TOTAL") == ["ATIVO TOTAL"]

    def test_list_passthrough(self) -> None:
        assert normalize_accounts(["ATIVO", "PASSIVO"]) == ["ATIVO", "PASSIVO"]

    def test_strings_with_newlines(self) -> None:
        assert len(normalize_accounts(["ATIVO\nTOTAL", "PASSIVO\tTOTAL"])) == 2
